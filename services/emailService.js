...
const emailConfig = {
  ...
  // Railway hosting optimizations (lightweight)
  connectionTimeout: 30000, // 30 seconds (reduced)
  greetingTimeout: 15000,   // 15 seconds (reduced)
  socketTimeout: 30000,     // 30 seconds (reduced)
  pool: false, // Disable pooling to save memory
  // Retry configuration
  retryDelay: 3000, // 3 seconds (reduced)
  retryAttempts: 2, // Reduced attempts
  // Debug mode for Railway
  debug: false, // Disable debug to save resources
  logger: false // Disable logger to save resources
};
...
const sendOTPEmail = async (email, otp, retryCount = 0) => {
  try {
    ...
    // Retry logic for Railway hosting (lightweight)
    if (retryCount < 1 && (error.code === 'ETIMEDOUT' || error.code === 'ECONNRESET' || error.message.includes('timeout'))) {
      console.log(`🔄 Retrying OTP email in 3 seconds... (attempt ${retryCount + 2})`);
...
const sendVerificationEmail = async (email, verificationToken, retryCount = 0) => {
  try {
    ...
    // Retry logic for Railway hosting (lightweight)
    if (retryCount < 1 && (error.code === 'ETIMEDOUT' || error.code === 'ECONNRESET' || error.message.includes('timeout'))) {
      console.log(`🔄 Retrying verification email in 3 seconds... (attempt ${retryCount + 2})`);
      await new Promise(resolve => setTimeout(resolve, 3000));
...
const sendPasswordResetEmail = async (email, resetToken, retryCount = 0) => {
  try {
    ...
    // Retry logic for Railway hosting (lightweight)
    if (retryCount < 1 && (error.code === 'ETIMEDOUT' || error.code === 'ECONNRESET' || error.message.includes('timeout'))) {
      console.log(`🔄 Retrying password reset email in 3 seconds... (attempt ${retryCount + 2})`);
      await new Promise(resolve => setTimeout(resolve, 3000));
